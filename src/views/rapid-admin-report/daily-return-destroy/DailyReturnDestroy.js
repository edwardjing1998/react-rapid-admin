...
import './DailyReturnDestroy.scss';
import { clientsJson, dailyReturnDestroyData } from './data.js';
...
const DailyReturnDestroy = () => {
  const [selectedOption, setSelectedOption] = useState(['ALL']);
  const [fromDate, setFromDate] = useState('');
  const [toDate, setToDate] = useState('');
  const [clientOptions, setClientOptions] = useState([]);
  const [rowData, setRowData] = useState([]);
  const [selectedDispositions, setSelectedDispositions] = useState(['D', 'B', 'S']);
  ...
  useEffect(() => {
    ...
    filterData(['ALL'], oneMonthAgo, today, ['D', 'B', 'S']);
  }, []);
  ...
  const filterData = (clientIds, from, to, dispositions) => {
    ...
    if (clientIds.includes('ALL')) {
      filteredData = Object.values(dailyReturnDestroyData).flat();
    } else {
      clientIds.forEach(cid => {
        filteredData.push(...(dailyReturnDestroyData[cid] || []));
      });
    }
  ...
  const handlePreview = () => {
    ...
    const dispositions = selectedDispositions.length === 0 ? ['D', 'B', 'S'] : selectedDispositions;

    if (selectedOption.length === 0) {
      setSelectedOption(['ALL']);
    }
    if (selectedDispositions.length === 0) {
      setSelectedDispositions(['D', 'B', 'S']);
    }
...
export default DailyReturnDestroy;
